/**
 * The Copilot pipeline end to end, against the real content script and
 * real Chrome DOM/event semantics. Docs/planning/phase_3_gate_actuation_verification.md
 * task 3.8 (native-setter type, full pointer click) and task 3.9
 * (navigation verbs from the service worker).
 */
...
test('type on a React-style controlled input lands and survives the framework\'s own re-render', async ({ context, extensionId }) => {
  ...
  const result: any = await agentAct(popup, tabId, 'type "Mohd Taha" into the full name field');
...
test('click on a pointerdown-only custom control fires the handler', async ({ context, extensionId }) => {
  ...
  const result: any = await agentAct(popup, tabId, 'click Continue');
  expect(result.data.phase).toBe('done');
...
test('type into a contenteditable rich editor lands via execCommand insertText', async ({ context, extensionId }) => {
  ...
  const result: any = await agentAct(popup, tabId, 'type "Hello there" into the message field');
  expect(result.data.phase).toBe('done');
...
test('navigate to an ungranted origin is refused OUT_OF_SCOPE and the tab does not move', async ({ context, extensionId }) => {
  ...
  const result: any = await agentAct(popup, tabId, 'go to http://localhost:5601/basic-form.html');
...
test('history_back returns to the previous URL and verifies via location', async ({ context, extensionId }) => {
  ...
  const result: any = await agentAct(popup, tabId, 'go back');
  expect(result.data.phase).toBe('done');
...
test('a click on a real cookie-banner-covered button is refused OBSCURED, against real Chrome layout', async ({ context, extensionId }) => {
  ...
  // Dismiss the cookie banner and try again — now it lands.
  await page.locator('#accept-cookies').click();
  const after: any = await agentAct(popup, tabId, 'click Continue');
