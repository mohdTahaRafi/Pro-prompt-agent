...
## 16. Implementation Addendum (written during Phase 4 itself)

Nine places where implementation surfaced a gap or ambiguity in §1–§15 above, decided and written back here rather than left silent, per this repo's spec-correction practice (see Phase 3's four corrections for precedent). 16.8 and 16.9 were found during the post-implementation acceptance audit, not during the original build.

**16.1 — `route()` must call `minimise()` for Class A too, not only Class B.** §4's `route()` sketch guards the `minimise()` call with `req.disclosureClass === 'B'`. But §7.2's `minimise()` also implements a Class A branch (the raw-page-text assertion), which would then be unreachable — planner.ts always sets `disclosureClass: 'A'`, and nothing would ever exercise it. Corrected: `route()` calls `minimise()` whenever `req.disclosureClass` is set AND the resolved chain can reach a remote engine, covering both classes. Implemented in `lib/model/router.ts`; asserted in `tests/unit/minimise.spec.ts`.

**16.2 — No bake-off-validated default planner model exists.** §3.1 says "The default planner and runner-up named in `bakeoff_phase2.md` are what this phase configures as shipped defaults" — but `bakeoff_phase2.md`'s own Decision section states plainly: *"No real candidate was measurable in this environment — no default planner is named."* Only the deliberate 1.5B control was reachable there. This phase ships `qwen2.5:14b` as the interim default anyway (`lib/model/engines/ollama.ts`'s `DEFAULT_PLANNER_MODEL`) because §3.2's disclosure copy and §11's Milestone Definition both name it explicitly and a planner tier with no default at all cannot ship — but it is NOT a bake-off-validated choice, and is documented as such at its declaration site. Re-running the bake-off against it specifically, once a real dev machine can pull it, is unresolved follow-up work, not assumed done.

**16.3 — Which tier serves the four direct-path text verbs.** §2's table says refactor/generate/score/summarise are "migrated onto the judge/planner tiers" without saying which agent gets which. Decided: all four route on the **judge** tier, never planner — the planner tier's chain has NO in-browser engine in either posture (§3.1), so routing these there would mean a Local-only user with no Ollama installed loses refactor/generate/score entirely, a real regression from the Phase 1 cascade's WebGPU-first behaviour. §3.7.10 also states these verbs bypass "the kernel, the planner, or the gate" entirely, which rules planner out on its own terms. Documented in `lib/agents/text-tier.ts`'s header; implemented in `lib/agents/{refactor,generator,comprehension,scorer,context-update-agent}.ts`.

**16.4 — `RouteRequest` needed a `runId` field the doc's sketch never declared.** §5.4's `remote.ts` and §4.2's fallback-journaling both read `req.runId` to journal `inference.remote`/`inference.fallback` against a run, but §4's `RouteRequest` interface never declares that field. Added as `runId?: number` in `lib/model/router-types.ts`, undefined for the direct-path text agents and inline completion (neither of which has a run to journal against).

**16.5 — `lib/model/engines/prompt-api.ts` is a service-worker-side proxy, not the session host.** §5.1's code sample is headed "— offscreen document", implying the warm-base/`clone()` logic lives in that file directly. But `lib/model/router.ts`'s `route()` runs in the service worker (called from `entrypoints/background.ts`'s message handlers, same as `gate()`/`verify()`), and `LanguageModel` is reachable only from the offscreen document (Phase 1's Q11 spike — the same reason WebLLM has always lived in `entrypoints/offscreen/main.ts`). So, mirroring how §5.2 explicitly describes `webllm.ts` as "a wrapper... over the retained offscreen host": `lib/model/engines/prompt-api.ts` is the service-worker-side proxy `lib/model/router.ts`'s `CHAINS` references, sending one message per call; the real `ensureBase()`/`clone()`/`prompt()` logic was added to `entrypoints/offscreen/main.ts` alongside the existing WebLLM host, behind new `PROMPT_API_AVAILABILITY`/`PROMPT_API_INFER`/`PROMPT_API_ABORT` message types. `PlanStepSchema` also gained an undeclared-by-the-doc `targetHint: {role, name}` field, populated by `planner.ts` itself (never trusted from the model), so `step-resolver.ts`'s stage-1 "same role and name" check (§8.4) has something concrete to compare against.

**16.6 — Q6 (the inline model bench, task 4.16) could not be measured in this environment.** No real Chrome build with the Prompt API enabled was available this session — see `Docs/planning/inline_bench_phase4.md` for the full account and what WAS verified mechanically in its place (the warm-session/clone mechanism itself, driven against a mocked `LanguageModel`).

**16.7 — §12's model-latency and SC-11 rows are unmeasured for the same reason `baseline_phase1.md` already documents.** That file recorded, before this phase's changes, that this sandbox has no usable WebGPU (software-adapter shader compile failure), no usable Ollama (CORS rejects `chrome-extension://` origins without root), and no configured remote key. Nothing about that changed this session, so Planner/Judge/Inline latency and the SC-11 comparison are not measured here — see the session's report for exactly what WAS verified (structural router/minimise/repair guarantees, all 468 unit tests, all 25 e2e specs including the two latency benches on the *existing* gate/actuation path, the production build, and the offscreen bundle size).

**16.8 — Task 4.2's literal acceptance wording ("a local-only planner request with Ollama down returns `NO_ENGINE_FOR_TIER`") does not hold, by design.** `CHAINS.planner['local-only']` is `[ollamaEngine]` — never an empty array — so an Ollama-down planner call legitimately calls `fetch()` (that is how it discovers Ollama is down) and returns a connection-failure code from that attempt, not `NO_ENGINE_FOR_TIER`. `NO_ENGINE_FOR_TIER` is only reachable when a `(tier, posture)` pair's chain is empty, which is true of no entry in `CHAINS` today — it exists for a future tier/posture with no engine at all. `tests/unit/router.spec.ts` tests the two real, separate properties task 4.2 actually cares about instead: (1) `NO_ENGINE_FOR_TIER` + zero network calls, on a chain forced empty for the test; (2) an Ollama-down Local-only planner call never reaches any host other than `localhost:11434` — the actual boundary guarantee. Found and documented in the test file's own header comment during implementation; surfaced here during the acceptance audit because it was not yet written back to this doc.

**16.9 — `tests/bench/text-ops.bench.ts` (the SC-11 baseline script referenced by task 4.15 and §12) is stale against this phase's message-type changes and needs a small update before it can be run on a real dev machine.** It calls `GET_PROVIDER_STATUS` and `SET_ACTIVE_PROVIDER` (§2, §16.3's table) to probe reachability and select a provider before timing `REFACTOR`/`SCORE`/`GENERATE`/`SAVE_CONTEXT` — both message types were deleted in this phase (replaced by `GET_POSTURE_CAPABILITY`, `SET_OLLAMA_CONFIG`, `SET_REMOTE_CONFIG`; see `entrypoints/background.ts`). Run unmodified today, it would not fail loudly — it would report every provider "not reachable" for the wrong reason (an unrecognised message type, not the real WebGPU/Ollama-CORS/no-key gaps `baseline_phase1.md` already documents) and silently produce a misleading all-"not measured" report. The `REFACTOR`/`GENERATE`/`SCORE`/`SAVE_CONTEXT` message payload shapes themselves are unchanged and need no update. This is separate from, and in addition to, §16.7's infra gap: even with a real GPU/Ollama/key available, this script would need its provider-probe and provider-selection calls rewritten against the new message types first. Found during the post-implementation acceptance audit; not fixed in this session, since it is a bench-tooling update rather than a product-code gap and was out of the audit's scope. Tracked as follow-up work for whoever next has a real dev machine to run §12's live numbers on.
